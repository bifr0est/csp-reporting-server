...
def get_rabbitmq_connection():
    ...
    try:
        ...
        logging.error(
            f"Failed to connect to any configured RabbitMQ node: {e}", exc_info=True
        )
...
def receive_csp_report():
    ...
    if not (
        content_type == "application/csp-report" or content_type == "application/json"
    ):
    ...
    try:
        ...
        try:
            ...
            if (
                "csp-report" not in report_json
                and "violated-directive" not in report_json
            ):
        ...
        logging.info(
            f"Attempting to publish valid CSP report (size: {len(report_data_bytes)} bytes)"
        )
        ...
        try:
            with get_rabbitmq_connection() as connection:
                ...
                logging.info(
                    f"Successfully published report to RabbitMQ queue: {QUEUE_NAME}"
                )

        except AMQPConnectionError as e:
            logging.error(
                f"RabbitMQ connection error during publish: {e}", exc_info=True
            )
            ...
            Exception
        ) as e:  # Catch other errors from get_rabbitmq_connection or publish
            logging.error(
                f"Unexpected error sending report to RabbitMQ: {e}", exc_info=True
            )
        ...
        logging.error(
            f"Error processing request (pre-queue operation): {e}", exc_info=True
        )
