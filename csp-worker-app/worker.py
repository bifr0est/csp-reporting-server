...
RABBITMQ_CONNECTION_ATTEMPTS_PER_NODE = int(
    os.environ.get("RABBITMQ_CONNECTION_ATTEMPTS_PER_NODE", 3)
)
...
def init_elasticsearch_client():
    ...
    for attempt in range(ES_CONNECTION_RETRIES):
        try:
            logging.info(
                f"Attempting to connect to Elasticsearch (attempt {attempt + 1}/{ES_CONNECTION_RETRIES})..."
            )
            temp_es_client = Elasticsearch(ELASTICSEARCH_HOSTS, request_timeout=10)
            if temp_es_client.ping():
                es_client = temp_es_client
                logging.info(
                    f"Successfully connected to Elasticsearch on attempt {attempt + 1}."
                )
                return True
            else:
                logging.warning(
                    f"Elasticsearch ping failed on connection attempt {attempt + 1}."
                )

        except es_exceptions.ConnectionError as e:
            logging.warning(
                f"Elasticsearch connection attempt {attempt + 1} failed: {e}"
            )
        except Exception as e:
            logging.error(
                f"Unexpected error initializing Elasticsearch client on attempt {attempt + 1}: {e}",
                exc_info=True,
            )

        if attempt + 1 < ES_CONNECTION_RETRIES:
            logging.info(
                f"Retrying Elasticsearch connection in {ES_RETRY_DELAY} seconds..."
            )
            time.sleep(ES_RETRY_DELAY)
    logging.error(
        f"Failed to connect to Elasticsearch after {ES_CONNECTION_RETRIES} attempts."
    )
    return False
...
def flush_es_batch(channel):
    ...
    if not es_client or not es_client.ping():
        logging.error(
            "Elasticsearch client not available or ping failed during flush. Attempting to NACK and requeue messages."
        )
        for delivery_tag, _ in es_bulk_batch:
            try:
                ...
                logging.error(
                    f"Failed to NACK message (delivery_tag: {delivery_tag}) during ES down flush: {e_nack_conn_down}"
                )
        es_bulk_batch.clear()
        if not es_client and not init_elasticsearch_client():
            logging.error(
                "Failed to re-initialize ES client during flush after it was down."
            )
        return False
    ...
    delivery_tags_for_bulk = [
        delivery_tag for delivery_tag, _ in current_batch_to_process
    ]
    ...
    try:
        for i, (ok, result_item) in enumerate(
            es_helpers.streaming_bulk(
                client=es_client,
                actions=actions_for_bulk,
            ...
            if ok:
                ...
                logging.debug(
                    f"Successfully indexed doc (delivery_tag: {delivery_tag}, ES_id: {doc_id})."
                )
        ...
        logging.error(
            f"Elasticsearch Connection Error during streaming_bulk: {es_conn_err}",
            exc_info=True,
        )
        ...
        logging.error(
            f"Unexpected error during Elasticsearch streaming_bulk: {e}", exc_info=True
        )
    ...
    for delivery_tag in failure_nacks_requeue_false:
        try:
            ...
            logging.error(
                f"Error NACKing (requeue=False) message {delivery_tag}: {e_nack_f}"
            )
    ...
    for delivery_tag in failure_nacks_requeue_true:
        try:
            ...
            logging.error(
                f"Error NACKing (requeue=True) message {delivery_tag}: {e_nack_t}"
            )
...
def process_report_message(ch, method, properties, body):
    ...
    report_identifier = (
        f"delivery_tag: {method.delivery_tag}, approx_body_start: {body[:60]}"
    )
    ...
    try:
        ...
        index_name = (
            f"{ELASTICSEARCH_INDEX_PREFIX}-{datetime.now(UTC).strftime('%Y.%m.%d')}"
        )
        ...
        if len(es_bulk_batch) >= BULK_MAX_SIZE:
            logging.info(
                f"Batch size {len(es_bulk_batch)} reached MAX_SIZE ({BULK_MAX_SIZE}). Flushing..."
            )
            if flush_es_batch(ch):
                pass  # Timer reset is handled by main loop after flush
    except json.JSONDecodeError:
        logging.error(
            f"JSON Decode Error for {report_identifier}. Discarding message: {body!r}",
            exc_info=True,
        )
        ch.basic_nack(delivery_tag=method.delivery_tag, requeue=False)
    except UnicodeDecodeError:
        logging.error(
            f"Unicode Decode Error for {report_identifier}. Discarding message (raw bytes): {body!r}",
            exc_info=True,
        )
        ch.basic_nack(delivery_tag=method.delivery_tag, requeue=False)
    except Exception as e:
        logging.error(
            f"Critical unexpected error preparing report for batch {report_identifier}: {e}",
            exc_info=True,
        )
        ch.basic_nack(delivery_tag=method.delivery_tag, requeue=False)
...
def get_rabbitmq_connection_params_list():
    ...
    for node_str in node_strings:
        ...
        try:
            ...
            port = 5672
            logging.warning(
                f"Could not parse port for RabbitMQ node '{node_str}'. Defaulting to host '{host}' and port {port}."
            )
...
def main():
    ...
    if not init_elasticsearch_client():
        logging.warning(
            "Elasticsearch client failed to initialize on startup. Worker will attempt to initialize/re-check."
        )
    ...
    while not shutdown_requested:
        try:
            if not rmq_connection or rmq_connection.is_closed:
                ...
                if (
                    hasattr(rmq_connection, "_impl")
                    and rmq_connection._impl
                    and hasattr(rmq_connection._impl, "params")
                ):
                    connected_params = rmq_connection._impl.params
                    logging.info(
                        f"Successfully connected to RabbitMQ node: {connected_params.host}:{connected_params.port}"
                    )
                else:
                    ...
                    logging.info(
                        "Successfully connected to RabbitMQ (node details unavailable via _impl.params)."
                    )
                ...
                channel.basic_consume(
                    queue=QUEUE_NAME, on_message_callback=process_report_message
                )
                logging.info(
                    f"Worker set up to consume from queue: {QUEUE_NAME}. Max batch: {BULK_MAX_SIZE}, Timed flush: {BULK_MAX_SECONDS_FLUSH}s."
                )
                last_flush_time = time.time()

            if (
                rmq_connection
                and rmq_connection.is_open
                and channel
                and channel.is_open
            ):
                rmq_connection.process_data_events(
                    time_limit=PROCESS_DATA_EVENTS_TIMEOUT
                )
            else:
                logging.warning(
                    "RabbitMQ connection or channel is not open. Delaying before retry."
                )
            ...
            if es_bulk_batch and (
                current_time - last_flush_time >= BULK_MAX_SECONDS_FLUSH
            ):
                logging.info(
                    f"Timed flush triggered ({BULK_MAX_SECONDS_FLUSH}s). Batch size: {len(es_bulk_batch)}"
                )
            ...
            logging.warning(
                f"Failed to connect to any RabbitMQ node in list: {amqp_err}. Retrying in {RABBITMQ_RETRY_DELAY}s..."
            )
            ...
            logging.error(
                f"RabbitMQ Channel Error in main loop: {amqp_chan_err}", exc_info=True
            )
            if rmq_connection and rmq_connection.is_open:
                rmq_connection.close()
            rmq_connection, channel = None, None
            ...
            logging.info(
                "KeyboardInterrupt received in main loop. Initiating shutdown..."
            )
            shutdown_requested = True
        except Exception as e:
            logging.error(
                f"Worker main loop encountered an unexpected error: {e}. Retrying...",
                exc_info=True,
            )
            if rmq_connection and rmq_connection.is_open:
                rmq_connection.close()
    ...
    if rmq_connection and rmq_connection.is_open:
        ...
        try:
            ...
            logging.error(
                f"Error closing RabbitMQ connection during final shutdown: {e_close}"
            )
